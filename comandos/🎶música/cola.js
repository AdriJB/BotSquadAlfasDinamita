...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        ...
        if (!message.member.voice?.channel) 
            return message.reply(`❌ **Tienes que estar en un canal de voz para ejecutar este comando!**`);

        if (message.guild.me.voice?.channel && message.member.voice?.channel.id != message.guild.me.voice?.channel.id) 
            return message.reply(`❌ **Tienes que estar en el mismo canal de voz __QUE YO__ para ejecutar este comando!**`);
        ...
        async function paginacion() {
            ...
            collector.on("end", () => {
                ...
                embedpaginas.edit({content: `**El tiempo ha expirado! Escribe de nuevo** \`${prefix}queue\` **para volver a ver la cola de canciones!**`, embeds: [embeds[paginaActual].setFooter({ text: `Pagina ${paginaActual + 1} / ${embeds.length}` })], components: [embedpaginas.components[0]] }).catch(() => { }).then(msg => setTimeout(() => {
                    msg.delete(); 
                }, 10000));
            });
        }
